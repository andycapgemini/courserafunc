...
abstract class TweetSet {
   ...
   */
    ...
    def mrhelper(currentBest:Tweet): Tweet
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      ...
      right.filterAcc(p,left.filterAcc(p,t))
    }
  ...
  def mrhelper(currentBest:Tweet) = {
    val t:Tweet = if (currentBest==null || elem.retweets > currentBest.retweets) elem else currentBest
    left.mrhelper(right.mrhelper(t))
  }
}
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(t => apple.exists(term => t.text.contains(term)))
